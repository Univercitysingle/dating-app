...
const UserCard = ({ user, onSwipe }) => {
  ...
  return (
    <div
      className={`bg-white shadow-lg rounded-lg p-4 m-2 w-full md:w-1/2 lg:w-1/3 xl:w-1/4 flex flex-col transition-all duration-150 ease-in-out
    ...
    >
      ...
      <p className="text-gray-600 text-sm flex-grow">
        {user.bio ? user.bio.substring(0, 100) + '...' : 'No bio yet.'}
      </p>
...
function MatchesPage() {
    ...
    async (currentFilters) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              `Failed to fetch matches. Status: ${response.status}`
          );
        }
  ...
  const currentMatch =
    matches && matches.length > currentIndex ? matches[currentIndex] : null;
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col items-center">
      ...
      {isAdVisible && (
        <InterstitialAd isVisible={isAdVisible} onClose={handleAdClose} />
      )}

      {isLoading && (
        <p className="text-center text-gray-600 py-4">Loading matches...</p>
      )}
      ...
      {!isLoading &&
        ...
        currentIndex >= matches.length && (
          <p className="text-center text-gray-600 py-4">
            No more profiles in this batch. Try different filters or check back
            later!
          </p>
        )}
