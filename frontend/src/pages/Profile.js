  ...
  const handleHeroMediaUploadComplete = async (mediaUrl) => {
    ...
    try {
      ...
      setHeroMediaSaveStatus(
        `Error: ${error.data?.message || error.message || 'Failed to save hero media.'}`
      );
    }
  };
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col space-y-6">
        <section className="w-full relative">
          ...
          {heroMediaSaveStatus && (
            <p
              className={`text-sm text-center py-2 ${heroMediaSaveStatus.startsWith('Error:') ? 'text-red-500' : 'text-green-500'}`}
            >
              {heroMediaSaveStatus}
            </p>
          )}
