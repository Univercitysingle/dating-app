...
function HeroCaptureSection({ onUploadComplete }) {
  ...
  const captureImage = () => {
    ...
    canvas.toBlob(
      (blob) => {
        if (blob) {
          const file = new File([blob], 'hero-capture.jpg', {
            type: 'image/jpeg',
          });
          uploadToS3(file);
        } else {
  ...
  const uploadToS3 = async (file) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/upload-url?type=${encodeURIComponent(
          file.type
  ...
  useEffect(() => {
    if (media || !stream) {
      // if media was successfully set, or stream is not active
      stopCamera();
    }
  ...
  return (
    <div className="w-full p-4 flex flex-col items-center bg-gray-100 dark:bg-gray-700 rounded-xl shadow-md max-w-screen-md mx-auto">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md ${
            mode === 'image'
              ...
              : 'bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-400'
          }`}
        >
          Image Mode
        </button>
      ...
      {/* Canvas for image capture, initial size */}
      ...
      {media && (
        <div className="mt-4 w-full max-w-sm">
          ...
          media.endsWith('.jpeg') ||
          media.endsWith('.png') ||
          media.endsWith('.gif') ? ( // Basic check for image
            <img
              src={media}
              alt="Captured or Uploaded Hero Media"
